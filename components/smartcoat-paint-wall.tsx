...
export function SmartCoatPaintWall({ onColourChange }: { onColourChange?: (colour: PaintColour) => void }) {
  ...
  return <main className="smartcoat-shell">
    ...
    <section className="selection-bar" aria-live="polite"><div className="selection-swatch" style={{ backgroundColor: selected.hex }} /><div className="selection-copy"><span className="selection-label">SELECTED COLOUR</span><strong>{selected.name}</strong><span className="selection-meta"><code>{selected.hex}</code><span>RGB {selected.rgb}</span></span></div><a className={`use-colour${accepted ? ' accepted' : ''}`} href="#order" onClick={() => setAccepted(true)}>{accepted ? <><Check size={14} /> COLOUR ADDED TO PROJECT</> : <>USE COLOUR <span>↗</span></>}</a></section>
